...
use crate::{VmHandle, function::get_function, result::{GoLuaValueV2ArrayResult, GoNoneResult, GoValueV2Result, wrap_failable}, value_v2::{GoLuaValueV2, GoLuaValueV2Array}};
...
#[unsafe(no_mangle)]
pub extern "C" fn luago_create_thread(lua: VmHandle, func: GoLuaValueV2) -> GoValueV2Result {
    wrap_failable(|| {
        let lua = lua.get();
...
#[unsafe(no_mangle)]
pub extern "C" fn luago_reset_thread(lua: VmHandle, th: GoLuaValueV2, func: GoLuaValueV2) -> GoNoneResult {
    wrap_failable(|| {
        let lua = lua.get();
...
#[unsafe(no_mangle)]
pub extern "C" fn luago_thread_status(lua: VmHandle, th: GoLuaValueV2) -> u8 {
    wrap_failable(|| {
        let lua = lua.get();
...
#[unsafe(no_mangle)]
pub extern "C" fn luago_thread_sandbox(lua: VmHandle, t: GoLuaValueV2) -> GoNoneResult {
    wrap_failable(|| {
        let lua = lua.get();
...
#[unsafe(no_mangle)]
pub extern "C" fn luago_thread_resume(lua: VmHandle, th: GoLuaValueV2, args: GoLuaValueV2Array) -> GoLuaValueV2ArrayResult {
    wrap_failable(|| {
        let lua = lua.get();
        let Some(th) = get_thread(&lua, th) else {
...
#[unsafe(no_mangle)]
pub extern "C" fn luago_thread_resume_error(lua: VmHandle, th: GoLuaValueV2, error: GoLuaValueV2) -> GoLuaValueV2ArrayResult  {
    wrap_failable(|| {
        let lua = lua.get();
        let Some(th) = get_thread(&lua, th) else {
            return GoLuaValueV2ArrayResult::err("Function pointer is null".to_string());
